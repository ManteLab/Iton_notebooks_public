...
def iplot_data_2classes():
    ...
    def update_plot(corr_coeff=0, ortho_order="input / choice", orthogonalize = True):
        ...
        dist_normalized = np.hstack((dist1_normalized, dist2_normalized))

        # Linear regression to normalize beta
        #reg = LinearRegression().fit(data1, dist1_normalized)
        reg = LinearRegression().fit(data, dist_normalized)
        betas = reg.coef_
        ...
        with output_plot:
            ...
            axs[3].scatter(dist1_normalized, proj_input_tdr1, color=colors1, alpha=0.5, label = 'Input 1')
            axs[3].scatter(dist2_normalized, proj_input_tdr2, color=colors2, alpha=0.5, label = 'Input 2')
...
def visualize_correlated_data_first(corr_coeff=0):
    ...
    # Plot the data with color gradients in the first subplot
    axs.scatter(data1[:, 0], data1[:, 1], alpha=0.5, color='blue', label='Choice 1')
    axs.scatter(data2[:, 0], data2[:, 1], alpha=0.5, color='red', label='Choice 2')
    axs.set_xlabel('X1')
    axs.set_ylabel('X2')
    axs.set_xlim([-5, 5])
    axs.set_ylim([-5, 5])
