...
def plot_beta_norm(model_a, model_b, model_c, model_d):
    ...
    for id in range(4):
        ...
        if id == 1:
            model = model_b # model_d
        if id == 2:
            model = model_c # model_a
        if id == 3:
            model = model_d # model_c
...
def plot_pca_var_explained(model_a, model_b, model_c, model_d):
    ...
    for id in range(4):
        ...
        axes[id].plot(np.log(model['per_var_pca'][1:10]), label="choice direction")

        axes[id].set_xlabel("PCs")
        axes[id].set_ylabel("Log(Variance Explained)")
        axes[id].set_title(model['type'])
    ...
    for icol in range(2):
        ...
        for id1 in range(number_axes):
            ...
            for ip in range(np_len):
                ...
                ax.set_xlabel('time')
                if method == 'tdr':
                    ax.set_ylabel(f'{order_orth[id1]} projection')
                else:
